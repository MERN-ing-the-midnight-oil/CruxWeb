...
const GameBoard = () => {
	...
	const handleCellClick = (cell, rowIndex, cellIndex) => {
		if (cell && cell.clue) {
			const cluePath = `/clue${cell.clue.slice(4)}`;
			console.log("Clue path:", cluePath); // Logging the path
			setSelectedClue(cluePath);
	...
	const getCellStyle = (cell, rowIndex, cellIndex) => {
		const style = {
			width: "40px", // Ensure consistent sizing (optional)
			height: "40px", // Ensure consistent sizing (optional)
			border: "1px solid black", // Border for visibility (optional)
			display: "flex", // Center content (optional)
			alignItems: "center", // Center content (optional)
			justifyContent: "center", // Center content (optional)
		};

		if (!cell) {
			return { ...style, backgroundColor: "darkgrey", cursor: "default" }; // Inactive cells background
		}
		if (cell.clue) {
			return { ...style, backgroundColor: "lightblue", cursor: "pointer" }; // Clue cells background and pointer
		}
		const correctLetter = level3.grid[rowIndex][cellIndex]?.letter; // Retrieve the correct letter from the initial state
		return {
			...style,
			backgroundColor:
				cell.letter && cell.letter === correctLetter ? "green" : "white", // Change to green if correct
			cursor: cell.letter ? "text" : "default", // Different cursor for editable cells
		};
	};
	...
	return (
		<div>
			...
			<div className="clue-display">
				{selectedClue && (
					<img
						src={`images/level3/${selectedClue}.webp`}
						alt="Clue"
					/>
				)}
			</div>
